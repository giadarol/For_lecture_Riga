...
z_part = np.random.randn(n_part)*sigmaz+np.floor(np.linspace(0, 6, n_part))*L_ring/h
...
for ii, tt in enumerate(t_vect):
	...
	plt.savefig('tmpout/mpart_%s_a%04d.png'%(fname, ii), dpi=90)
...
os.system(' '.join([
    'ffmpeg',
    '-i tmpout/mpart_%s'%(fname)+'_a%04d.png',
    '-c:v libx264 -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2,setpts=4.*PTS"',
    '-profile:v high -level:v 4.0 -pix_fmt yuv420p -crf 22 -codec:a aac mpart_%s.mp4'%fname]))
